...
void Encoder_Init(ENCODER_CHANNEL ch)
{
	...
	// GIMA setup
	LPC_GIMA->CAPn_IN[encoder->timerNum][encoder->captureNum] = ( GIMA_ALT_SELECT(encoder->gimaInput) | GIMA_ENABLE_PULSE(1) | GIMA_ENABLE_SYNCH(1));

	// Enable timer clock and reset it
	Chip_TIMER_Init(encoder->timer);
	Chip_TIMER_PrescaleSet(encoder->timer, 1);	// Timer res = 204MHz/100 = 2.04MHz
	Chip_TIMER_TIMER_SetCountClockSrc(encoder->timer, TIMER_CAPSRC_RISING_CAPN, encoder->captureNum);
