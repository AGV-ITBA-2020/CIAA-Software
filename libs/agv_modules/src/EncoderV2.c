...
#define MAX_ENC_SAMPLES 10 //Cantidad m�xima de flancos positivos que hay entre cada per�odo de control
...
uint32_t EncoderV2_GetCount(ENCODER_CHANNEL_T ch)
{
	...
	encoder->measCount=0;
	NVIC_EnableIRQ( encoder->timerIrqAddr); //Habilita interrupci�n de vuelta
	return measAverage;
}
uint32_t EncoderV2_GetCountFiltered(ENCODER_CHANNEL_T ch,uint32_t minCount,uint32_t maxCount)
{
	...
	for(unsigned int i=0;i<encoder->measCount;i++)
	{
		...
		if(meas>=minCount && meas<=maxCount)	//Saca el promedio, pero las muestras que est�n fuera del rango de trabajo vuelan
			measAverage += meas;
			totalCount++;
	}
	if(totalCount>0)
...
void TIMER2_IRQHandler(void)
{
	...
	Chip_TIMER_Reset(encR.timer); //Esto deber�a ir en el caso en el que no se clearee solo el capture reg (no creo que pase)
...
{
	enc_config_t * encoder=getEncoderPointer(ch);
	if(encoder->measCount<MAX_ENC_SAMPLES) //En el caso que no llen� el buffer con mediciones ya
		encoder->measArray[(encoder->measCount)++]=Chip_TIMER_ReadCount(encoder->timer); //Pongo la medici�n obtenida
	Chip_TIMER_ClearCapture(encoder->timer, encoder->captureNum);
	//Ac� se podr�a poner disable del interrupt en caso de solo tomar una medici�n en vez de un promedio
}
